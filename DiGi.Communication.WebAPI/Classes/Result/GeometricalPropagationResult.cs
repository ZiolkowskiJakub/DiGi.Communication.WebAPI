...
namespace DiGi.Communication.WebAPI.Classes
{
    ...
    /// <para>AI-NOTE (scattering hit nesting): the Details drill-down of the consuming application is three steps deep and the payload mirrors it exactly. <see cref="AngularPowerDistributionResult.Cells"/> holds one <see cref="ScatteringHitCellResult"/> per populated azimuth and elevation bin, that cell holds one <see cref="ScatteringHitGroupResult"/> per distinct electrical properties, and that group holds the individual <see cref="ScatteringHitResult"/> instances. Flattening the group level back out would restore the earlier two step form and break the middle window.</para>
    ...
    public class GeometricalPropagationResult
    {
        ...
        /// <para>Only the scattering objects the hits of <see cref="ScatteringHitGroupResult.Hits"/> point at are described: an analyzed area holds thousands of scattering objects but the hits touch a handful of them, and a reference string is long enough that sending the whole lookup would dominate the payload.</para>
        /// </summary>
